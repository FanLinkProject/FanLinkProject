...
import { listByArtist as listReplays, deleteReplay } from "@/lib/replayApi";
import {
    isUpcoming,
                        ...
                        <div className="space-y-6">
                            ...
                            {vodLoading ? (
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    {vodList.map((vod) => (
                                        <div key={vod.replayId} className="group relative">
                                            <Link href={`/replay/${vod.replayId}`}>
                                            ...
                                            <button
                                                ...
                                                onClick={async (e) => {
                                                    ...
                                                    try {
                                                        await deleteReplay(vod.replayId);
                                                        setVodList((prev) => prev.filter((v) => v.replayId !== vod.replayId));
                                                    } catch (err) {
                                                        alert(err?.message || "삭제에 실패했습니다.");
                                                    }
                                                }}
                                                className="absolute top-2 left-2 size-8 rounded-full bg-black/70 text-white/60 hover:bg-red-600 hover:text-white flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100 z-10"
                                                title="삭제"
                                            >
