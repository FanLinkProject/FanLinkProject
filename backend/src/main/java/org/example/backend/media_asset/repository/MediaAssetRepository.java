...
public interface MediaAssetRepository extends JpaRepository<MediaAsset, Long> {
    ...
    List<MediaAsset> findByStatusAndExpiresAtBefore(MediaAssetStatus status, LocalDateTime now);

    // INITIATED 상태 중 만료된 항목을 조회한다.
    default List<MediaAsset> findExpiredInitiated(LocalDateTime now) {
