...
public class SettlementDashboardService {
     ...
     */
    public SettlementEstimateResponseDto getEstimatedAmount(Long artistId) {
        ...
        return SettlementEstimateResponseDto.of(totalEstimatedAmount);
    }

    /**
     * 2. [히스토리] 과거 지급 완료된 정산 목록 조회
     */
    public List<SettlementHistoryResponseDto> getSettlementHistory(Long artistId) {
        return settlementRepository.findAllByArtistIdOrderBySettledAtDesc(artistId)
                .stream()
                .map(SettlementHistoryResponseDto::from)
     ...
     */
    public List<SettlementDetailResponseDto> getSettlementDetails(Long artistId, Long settlementId) {
