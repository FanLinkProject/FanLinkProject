...
import org.example.backend.user.entity.Artist;
...
public class SettlementBatchConfig {
    ...
    @Bean
    public JpaPagingItemReader<Artist> artistReader() {
        return new JpaPagingItemReaderBuilder<Artist>()
                .name("artistReader")
                .entityManagerFactory(entityManagerFactory)
                .queryString("SELECT a FROM Artist a ORDER BY a.id ASC")
                .pageSize(10)
                .build();
    }
    ...
    public ItemProcessor<Artist, SettlementBatchData> settlementProcessor(
        ...
        return artist -> {
            ...
            boolean alreadySettled = settlementRepository.existsByArtistIdAndStartDateAndEndDate(
                    artist.getId(), startDate, endDate
            );
            if (alreadySettled) {
                log.info("Artist({})는 이미 {}~{} 정산이 완료되었습니다.", artist.getId(), startDate, endDate);
                return null; // Writer로 넘기지 않고 스킵
            }

            // 대기열 조회
            List<SettlementPending> pendings = pendingRepository.findAllByArtistIdAndDateRange(
                    artist.getId(), startDate.atStartOfDay(), endDate.atTime(23, 59, 59));
